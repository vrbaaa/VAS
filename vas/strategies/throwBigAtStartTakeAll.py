...
class ThrowBigAtStartTakeAll(BaseStategy, CyclicBehaviour):
    ...
    def handlePlayBigCard(self):
        filtered_cards = list(filter(lambda x: x.get_score() == 10 and x.get_sign() != '7', self.cards))
        if len(filtered_cards) > 0:
            ...
            filtered_cards = list(filter(lambda x: x.get_score()  != '7', self.cards))
            if len(filtered_cards) > 0:
                randic = random.randint(0,len(filtered_cards)-1)
                duplicate = filtered_cards[randic]
                index = next((i for i, x in enumerate(self.cards) if x.get_combo() == duplicate.get_combo()), -1)
    ...
    async def run(self):
        ...
        if message:
            ...
            if currPlayer=="throwbigatstarttakeall@localhost":
